...
use crate::ast_formatter::list::{list_overflow_yes, param_list_config, ArrayListConfig};
...
impl<'a> AstFormatter<'a> {
    pub fn expr(&mut self, expr: &ast::Expr, tail: Tail) -> FormatResult {
        match expr.kind {
            ...
            ast::ExprKind::MethodCall(_) => self.dot_chain(expr, tail, false),
            ...
            ast::ExprKind::Field(..) => self.dot_chain(expr, tail, false),
    ...
    fn match_(
        ...
        self.out.space()?;
        self.expr(scrutinee, Tail::new(|this| {
            this.out.space()?;
